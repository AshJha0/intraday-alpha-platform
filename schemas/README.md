...
| `alpha/` | `alpha_signal.schema.json` | 1 | Alpha model output: expected return (bps), confidence, horizon, `alpha_id`, the `feature_version` it was scored on. | `iap.alpha.base.AlphaModel` score rows (research reference), `iap.contracts.types.AlphaSignal` (`iap.mvp.alpha.LinearZAlpha`) / `iap::AlphaSignal` (`cpp/include/iap/alpha/alpha.hpp`) + trace record / `alpha::scoring::AlphaSignal` (`rust/alpha/src/scoring.rs`), `contracts::trace::AlphaSignal` / `com.iap.alpha.AlphaSignal`, `com.iap.trace.AlphaSignalRec` |
| `order/` | `order_request.schema.json` | 1 | Strategy order request handed to risk and the venue layer: order type (MARKET/LIMIT/IOC/FOK/PEG/MIDPOINT), side, price ticks, qty, TIF, strategy id. | `iap.risk.orders.OrderRequest` (the Python risk port; the Rust wire domains), `iap.backtest.engine` child orders (research) / `iap::ChildOrder` (`cpp/include/iap/execution/execution.hpp`) / `venue::OrderRequest` (`rust/venue/src/messages.rs`, also the risk engine's input) / `com.iap.risk.OrderRequest` |
| `execution/` | `execution_report.schema.json` | 1 | Venue execution report: status (NEW/PARTIAL/FILLED/CANCELED/REJECTED), fill price ticks, fill qty, fees, venue timestamps. | `iap.contracts.types.ExecutionReport` (`iap.mvp.adapters.SimulatorAdapter` from `iap.execution.types.Fill`), `iap.backtest.engine` fills (research) / `iap::Fill` (`cpp/include/iap/execution/execution.hpp`, the execution reference) + trace record / `venue::ExecutionReport` (`rust/venue/src/messages.rs`), `contracts::trace::ExecutionReport` / `com.iap.execution` fills, `com.iap.trace.ExecutionReportRec` |
| `risk/` | `risk_event.schema.json` | 1 | Hard-risk decision / audit record: decision (ALLOW/REJECT), `rule_id`, severity (INFO/WARN/BREACH), scope, the values compared. Byte-identical JSONL across languages (`tests/golden/expected_risk_audit.jsonl`). | `iap.risk.events.RiskEvent` (the Python reference port, byte-identical audit) / no C++ risk subsystem / `risk::RiskEvent` (`rust/risk/src/event.rs`, the reference) / `com.iap.risk.RiskEvent` (the port) |
| `risk/` | `risk_decision.schema.json` | 1 | Per-order hard-risk decision (ALLOW=1/REJECT=2/KILL=3, deciding `rule_id`, pinned `rule_index`, reason) — a `RiskEvent` joined with its order context. | `iap.contracts.types.RiskDecision` (`from_risk_event`; the MVP's `RiskEngineAdapter` builds it) / `iap::contracts` trace record / `contracts::trace::RiskDecision` / `com.iap.trace.RiskDecisionRec` (PaperTraces builds it from `com.iap.risk.RiskEvent`) |
| `portfolio/` | `portfolio_target.schema.json` | 1 | Portfolio solve output: `solver_status` (OPTIMAL/INFEASIBLE/MAX_ITER), objective, turnover, per-instrument legs (`target_qty`, `target_weight`, `expected_return_bps`, `prev_qty`), the portfolio / feature / model versions. | `iap.contracts.types.PortfolioTarget` (from `iap.portfolio.optimizer.PGDResult`; `iap.mvp.portfolio.SingleStockPortfolio`) / `iap::contracts` trace record / `contracts::trace::PortfolioTarget` / `com.iap.trace.PortfolioTargetRec` (from `com.iap.portfolio`) |
| `order/` | `parent_order.schema.json` | 1 | Strategy-level order for an execution algorithm (TWAP/VWAP/POV/IS): side, qty, decision/arrival/end ts, urgency, limit ticks, numeric `params`. | `iap.contracts.types.ParentOrder` (`iap.execution.algos.ParentOrder` is the simulator-side record, `iap.tca.fills.ParentOrder` the TCA-side view) / `iap::ParentOrder` + `iap::contracts` trace record / `contracts::trace::ParentOrder` / `com.iap.execution.ParentOrder`, `com.iap.trace.ParentOrderRec` |
| `order/` | `child_order.schema.json` | 1 | One slice of a parent order addressed to a venue (0 = SOR): qty, price ticks, `order_type` (the `order_request` enum), submit/expire ts, slice index. | `iap.contracts.types.ChildOrder` (`iap.execution.types.ChildOrder` is the simulator record) / `iap::ChildOrder` + trace record / `contracts::trace::ChildOrder`, `venue::OrderRequest` / `com.iap.execution.ChildOrder`, `com.iap.trace.ChildOrderRec` |
| `execution/` | `venue_decision.schema.json` | 1 | SOR decision for one child order: chosen `venue_id` (0 = NO_ROUTE), reason, every candidate's score (eligible, displayed price/qty, fees, rebate, commission, latency, rank). | `iap.contracts.types.VenueDecision` (`iap.mvp.adapters.SorAdapter` scores every candidate) / `iap::SorCandidate` table (`cpp/sor/`) + trace record / `contracts::trace::VenueDecision` / `com.iap.trace.VenueDecisionRec` (from `com.iap.sor`) |
| `tca/` | `tca_result.schema.json` | 1 | Per-parent-order TCA: fill rate, arrival / fill / VWAP / TWAP prices in ticks, Perold decomposition in bps (`implementation_shortfall = delay + trading + opportunity`, `trading = spread + impact + timing`), fees, slippage, participation, per-venue contribution, latency stats. | `iap.contracts.types.TCAResult` (from `iap.tca.tca.order_tca`; `iap.mvp.adapters.TcaAdapter`) / `iap::contracts` trace record / `contracts::trace::TcaResult` / `com.iap.trace.TCAResultRec` (from `com.iap.tca`) |
...
port; where it says "trace record", the language carries the schema as a
field of its `DecisionTrace` port (`com.iap.trace.*Rec`, `contracts::trace`,
`iap::contracts`), validated against the same golden instance; where it says
"no … subsystem", that is by design (see `docs/ARCHITECTURE.md` §2: Rust is
the risk reference with Java and Python as ports, C++ is the execution
reference with Java and Python as ports, C++ has no lifecycle). The typed
Python side is documented in [`../API_CONTRACTS.md`](../API_CONTRACTS.md).
