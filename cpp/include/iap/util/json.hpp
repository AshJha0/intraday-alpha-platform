// Minimal JSON parser for configuration and golden files (alpha params,
// venue configs, small tests/golden/*.json inputs). Header-only, no
// dependencies, throws std::runtime_error on malformed input. NOT a general
// JSON library: config-plane only, never on the event hot path.
